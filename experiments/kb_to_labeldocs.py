...
Usage: python kb_to_docs.py [DB_PATH] [URI_PREFIX]
"""

import sys
try:
    _, DB_PATH, URI_PREFIX = sys.argv
...
for i,s in enumerate(g.all_s()):
    ...
    for s_resolved in (set(get_redirs(s)) or [s]):
        s_uri = g.lookup_str(s_resolved)
        if not s_uri:
            continue
        uripart = util.quote(s_uri[1:-1].replace(URI_PREFIX, ''))
        ...
        for label_node in get_labels(s):
            ...
            if dtype == 'str' or dtype.startswith('@'):
                for label in util.label_variants(root):
                    ...
                    if label.isnumeric():
                        continue
                    label_entities.setdefault(label, set()).add(uripart)
                    

    
def stream():
    for label,uriparts in sorted(label_entities.items(), key=lambda x: -len(x[1])): 
        doc = {
            'label': label,
            'ids': list(uriparts),
        }
