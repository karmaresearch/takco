...
class PivotFinder:
    ...
    def find_pivot_cells(self, headers: Matrix) -> Iterable[Tuple[int, int]]:
    ...
    def find_longest_pivots(self, headers: Matrix) -> Iterable[Pivot]:
        """Yield longest pivots"""
        row_cols = defaultdict(set)
        for ri, ci in self.find_pivot_cells(headers):
            row_cols[ri].add(ci)
        ...
        for level, cols in row_cols.items():
            ...
            if colfrom <= colto and (colto - colfrom >= self.min_len - 1):
                yield Pivot(level, colfrom, colto, self.name, headers)

    def split_header(self, headers: Matrix, level: int, colfrom: int, colto: int):
        """Split the header containing the pivot"""
        return headers

    def unpivot(
        self, head, body, pivot, var_name: str = "_Variable", value_name: str = "_Value"
    ):
        """Unpivot a table
        ...
        if nhead > 1:
            # For tables with multiple header headers, the right columns get their own headers
            df = df.stack(level)
...
def yield_pivots(
    headertexts: Iterable[Collection[Collection[str]]], heuristics: List[PivotFinder]
):
...
def try_unpivot(table, pivot, named_heuristics):
    try:
        ...
        return Table(
            head=head, body=body, provenance={**table.provenance, "pivot": pivotmeta}
        )
    except Exception as e:
...
def get_colspan_fromto(headers: Matrix) -> List[List[Tuple[int, int]]]:
    ...
    for p, _ in pivot_size.most_common(1):
        ...
        if p.colfrom != p.colto and p.level > 0 and len(headertext) > 1:
            ...
            if len(spanning) == 1:
                ...
                p = Pivot(p.level, colfrom, colto, p.source, p.headers)
...
class RegexFinder(PivotFinder):
    ...
    def find_pivot_cells(self, headers: Matrix):
        for ri, hrow in enumerate(headers):
            for ci, cell in enumerate(hrow):
                if cell and self.pattern.match(cell.strip()):
                    yield ri, ci

    def split_header(self, headers, level, colfrom, colto):
        splitrow = []
        for ci, cell in enumerate(headers[level]):
        ...
        if splitrow1 != splitrow2:
            return tuple(headers[:0]) + (splitrow1, splitrow2) + tuple(headers[1:])
        else:
...
class SeqPrefix(PivotFinder):
    ...
    def find_pivot_cells(self, headers):
...
class SpannedRepeat(PivotFinder):
    ...
    def find_pivot_cells(self, headers):
        header_colspan, header_repeats = self.get_colspan_repeats(headers)
        header_fromto = get_colspan_fromto(headers)
        for ri, row in enumerate(headers):
...
class AgentLikeHyperlink(PivotFinder):
    ...
    def find_pivot_cells(self, headers):
        ...
        assert self.lookup is not None and self.kb is not None
        ents = self.lookup.lookup_cells(
            link.get_hrefs(headers, lookup_cells=self.lookup_cells)
        )
